...
const DetailedPricing = ({ data = {} }) => {
  ...
  return (
    <div className="w-full h-80 bg-gradient-to-br from-indigo-600 via-indigo-700 to-purple-800 flex items-center justify-center px-8 py-10">
      <div className="max-w-7xl w-full">
        ...
        <div className="grid grid-cols-3 gap-6">
          {plans.map((plan, index) => (
            <div key={index} className={`relative ${plan.popular ? 'bg-white text-indigo-600 scale-105 shadow-2xl' : 'bg-white/10 text-white backdrop-blur-lg'} rounded-2xl p-6 border-2 ${plan.popular ? 'border-yellow-400' : 'border-white/20'} transition-all hover:scale-105`}>
              {plan.popular && (
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-yellow-400 to-orange-500 text-white px-4 py-1 rounded-full text-xs font-bold shadow-lg">
                  MOST POPULAR
                </div>
              )}
              ...
              <button className={`${plan.popular ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white' : 'bg-white text-indigo-600'} px-6 py-3 rounded-xl font-bold hover:opacity-90 transition-all w-full shadow-lg`}>
                {plan.buttonText}
              </button>
            </div>
          ))}
